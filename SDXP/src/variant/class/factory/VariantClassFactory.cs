...
public static class VariantClassFactory
{
		...
		VariantClass variantClass)
	{
		...
		var declaredConstants = new Constant[length];
		...
		for (int i = 0; i < length; i++)
		{
			...
			declaredConstants[i] = new Constant(name, variantClass, value);
		}
		
		variantClass.DeclaredConstants = declaredConstants.ToArray();
	}
		...
		[AllowNull] MethodBinder instanceMethodBinder)
	{
		...
		var declaredFunctions = new List<Function>();
		...
		variantClass.DeclaredFunctions = declaredFunctions.ToArray();
	}
		...
		[AllowNull] PropertyBinder instancePropertyBinder)
	{
		...
		var declaredProperties = new List<Property>();
		...
		variantClass.DeclaredProperties = declaredProperties.ToArray();
	}
		...
		HashSet<string> memberNames)
	{
		...
		var declaredConstants = new List<Constant>();
