...
public class UserFunction : Function, ISourceLocated
{
	...
	{
		return UnsafeInvoke(InitializeVariables(self, args));
	}
	
	public Variable[] InitializeVariables(Variant self, Variant[] callerArgs)
	{
		...
		for (; i < callerArgs.Length; i++)
		{
			variables[variableIndex++] = new Variable(arguments[i].VariantClass, callerArgs[i]);
		}
		...
		{
			variables[variableIndex++] = new Variable(arguments[i].VariantClass, DefaultValues[defaultValueIndex++]);
		}

		return variables;
	}

	public Variant UnsafeInvoke(Variable[] variables)
	{
