...
public class Tokenizer : IDisposable
{
    ...
    private void CreateToken(SourceLocation location, TokenType tokenType, Variant value = default)
    {
    ...
    private void CreateAlphaNumericToken(SourceLocation location, char initial)
    {
        ...
        if (GlobalMaps.KeywordMap.TryGetValue(str, out TokenType keywordType))
        {
            CreateToken(location, keywordType);
            return;
        }

        if (GlobalMaps.LiteralMap.TryGetValue(str, out Variant literal))
        {
