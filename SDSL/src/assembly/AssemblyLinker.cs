...
public class AssemblyLinker
{
	...
	public AssemblyLinker(VariantAssembly assembly)
	{
		_assembly = assembly;
	}
	...
	private void BuildInheritanceTree(VariantClass rootClass)
	{
		...
		var userClassStack = new Stack<UserClass>();
		...
		while (currentClass != null)
		{
			...
			if (currentClass is UserClass { InstanceFields: null } userClass)
			{
				// Resolve prototype base class type
				LinkBaseClass(userClass);
				
				// Add to the allocation stack
				userClassStack.Push(userClass);
			}
	...
	private static void AllocateInstanceFields(Stack<UserClass> userClassStack)
	{
		...
		UserClass firstClass = userClassStack.Peek();
		...
		if (firstClass.BaseClass is UserClass baseClass)
		{
	...
	private void LinkProperties(VariantClass variantClass)
	{
		foreach (Property property in variantClass.LocalProperties)
		{
			...
			case null:
				valueClass = null;
				break;
			case IncompleteClass.ImplicitName:
				valueClass = IncompleteClass.Implicit;
				break;
			default:
	...
	private void LinkBaseClass(UserClass userClass)
	{
